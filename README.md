...
- The config now auto-discovers **all registry internals** and exposes them under `internals` using `registry.<registry_id>.<entry_id>` keys.
- The Mod Menu UI includes **All Internals (Unsafe)** for raw low-level editing.
- This intentionally removes safety clamping on damage resistance values for full developer freedom.
