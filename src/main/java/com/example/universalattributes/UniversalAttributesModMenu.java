...
public class UniversalAttributesModMenu implements ModMenuApi {
    ...
    private Screen createConfigScreen(Screen parent) {
        ...
        addInternalsCategory(builder, eb, Text.translatable("text.universalattributes.section.internals"), config.internals, false);
        addInternalsCategory(builder, eb, Text.translatable("text.universalattributes.section.internals_all"), config.internals, true);
    ...
    private void addAttributeCategory(ConfigBuilder builder, ConfigEntryBuilder eb, Text title, Map<String, ModConfig.AttributeRule> attributes, AttributeBucket bucket) {
        ...
        attributes.entrySet().stream()
                ...
                .forEach(entry -> {
                    ...
                    Text[] tooltip = tooltipForId(entry.getKey());
    ...
    private void addInternalsCategory(ConfigBuilder builder, ConfigEntryBuilder eb, Text categoryTitle, Map<String, ModConfig.InternalRule> internals, boolean registryOnly) {
        ConfigCategory category = builder.getOrCreateCategory(categoryTitle);
        internals.entrySet().stream()
                .filter(entry -> registryOnly == entry.getKey().startsWith("registry."))
                .sorted(Comparator.comparing(Map.Entry::getKey))
                .forEach(entry -> {
                    ModConfig.InternalRule rule = entry.getValue();
                    Text shortLabel = shortLabel(rule.displayName, entry.getKey());
                    Text[] tooltip = new Text[]{
                            Text.literal("ID: " + entry.getKey()),
                            Text.literal(rule.description == null ? "" : rule.description)
                    };
                    category.addEntry(eb.startBooleanToggle(shortLabel.copy().append(" • Enabled"), rule.enabled)
                            .setTooltip(tooltip)
                            .setSaveConsumer(v -> rule.enabled = v)
                            .build());
                    category.addEntry(eb.startDoubleField(shortLabel.copy().append(" • Numeric"), rule.numericValue)
                            .setTooltip(tooltip)
                            .setSaveConsumer(v -> rule.numericValue = v)
                            .build());
                    category.addEntry(eb.startBooleanToggle(shortLabel.copy().append(" • Boolean"), rule.booleanValue)
                            .setTooltip(tooltip)
                            .setSaveConsumer(v -> rule.booleanValue = v)
                            .build());
                });
    }

    private Text[] tooltipForId(String id) {
        String namespace = namespaceOf(id);
        return new Text[] {
                Text.literal("ID: " + id),
                Text.literal("Mod: " + namespace)
        };
    }
